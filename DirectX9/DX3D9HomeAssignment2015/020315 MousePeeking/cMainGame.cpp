...
void cMainGame::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	case WM_RBUTTONUP:{
		...
		if (m_bCharacterSelected && m_pSkinnedMeshObject){
			...
			D3DXVECTOR3 crossed = m.GetCrossSectionWithPlain(&ray, m_pPlain->GetVerticesPoints(), isCrossed);
			if (isCrossed){
				if (m_pSkinnedMeshObject){
					...
					cActionSeq* pActionSeq = new cActionSeq;
					cActionMove* pActionMove = new cActionMove;
					pActionMove->SetFrom(m_pSkinnedMeshObject->GetTransform()->GetPosition());
					pActionMove->SetTo(crossed);

					float length = D3DXVec3Length(&(m_pSkinnedMeshObject->GetTransform()->GetPosition() - crossed));

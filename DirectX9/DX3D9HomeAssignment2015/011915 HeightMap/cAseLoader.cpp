...
cFrame* cAseLoader::Load(std::string& sFolder, std::string& sFileName, stSceneInfo& scene)
{
	m_sFolder = sFolder;
...
void cAseLoader::ProcessTMAnimation(IN OUT cFrame* pFrame){
	int nLevel = 0;
	do
	{
		char* szToken = GetToken();
		...
		{
			ProcessPosTrack(pFrame);
		}
		else if (IsEqual(szToken, ID_ROT_TRACK)){
			ProcessRotTrack(pFrame);
		}
		else if (IsEqual(szToken, ID_CONTROL_ROT_TCB)){
			ProcessTCBRotTrack(pFrame);
		}
	} while (nLevel > 0);
}

void cAseLoader::ProcessPosTrack(IN OUT cFrame* pFrame){
	int nLevel = 0;
	do
	{
		char* szToken = GetToken();
...
void cAseLoader::ProcessRotTrack(IN OUT cFrame* pFrame){
	int nLevel = 0;
	do
	{
		char* szToken = GetToken();
		...
		else if (IsEqual(szToken, ID_ROT_SAMPLE))
		{
			...
			s.q.x = s.q.x * sin(s.q.w / 2.0f);
			s.q.y = s.q.y * sin(s.q.w / 2.0f);
			s.q.z = s.q.z * sin(s.q.w / 2.0f);
			s.q.w = cos(s.q.w / 2.0f);
...
void cAseLoader::ProcessTCBRotTrack(IN OUT cFrame* pFrame){
	int nLevel = 0;
	do
	{
		char* szToken = GetToken();

...
void cPlayer::Setup(cMainGame* game, iMap* map){
	m_pGame = game;
	m_pMap = map;
...
void cPlayer::Update(float delta){
	...
	if (GetKeyState(VK_CONTROL) & 0x8000 && m_eCurrAnim != E_ANIM_ATTACK){
		...
		if (GetKeyState('W') & 0x8000)
		{
			...
			vPosition = m_vPosition + (m_vForward * m_fSpeed * delta);
		}
	...
	{
		bool isLand = false;
		float fHeight = m_pMap->GetHeight(isLand, &vPosition);
		if (isLand)
		{
			m_vPosition = vPosition;
			m_vPosition.y = fHeight + m_fHeight;
		}
	}
	else
	{

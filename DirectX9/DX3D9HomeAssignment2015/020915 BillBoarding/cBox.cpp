...
void cBox::init(){
	...
	for (UINT j = 0; j < plane.size(); j++){
		...
		m_vecVertex.push_back(v);
		m_vecSuface.push_back(v.p);
		m_vecSufaceDebug.push_back(v);
	}
	...
	for (UINT i = 0; i < m_vecSuface.size(); i++){
		D3DXMatrixScaling(&matScale, m_vSize, m_vSize, m_vSize);
		D3DXVec3TransformCoord(&m_vecSuface[i], &m_vecSuface[i], &matScale);
	}
...
void cBox::render(){
	if (m_hp > 0){
		...
		//D3DXMatrixScaling(&matS, 0.8f, 0.001f, 0.8f);
		//g_pD3DDevice->SetTransform(D3DTS_WORLD, &matS);
		//g_pD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST,
		//	m_vecSufaceDebug.size() / 3,
		//	&m_vecSufaceDebug[0],
		//	sizeof(ST_PC_VERTEX));

		D3DXMATRIXA16 matWorld, matS, matR, matT, matI;
		D3DXMatrixTranslation(&matT, m_vPosition.x, m_vPosition.y - 2.0f, m_vPosition.z);
		D3DXMatrixScaling(&matS, 1.0f, 0.001f, 1.0f);
		matWorld = matS*matT;
...
void cBox::setPositionByGrid(GRIDPOSITION& gridpos){
	...
	m_vPosition = D3DXVECTOR3(gridpos.x * m_vSize, m_vPosition.y - 0.75f, gridpos.z*m_vSize);
	D3DXMATRIXA16 matT;
	for (UINT i = 0; i < m_vecSuface.size(); i++){

		D3DXMATRIXA16 matWorld, matS, matR, matT;
		D3DXMatrixTranslation(&matT, m_vPosition.x, m_vPosition.y - 2.00f, m_vPosition.z);
		D3DXMatrixScaling(&matS, 1.0f, 0.2f, 1.0f);
		matWorld = matS*matT;
		D3DXVec3TransformCoord(&m_vecSuface[i], &m_vecSuface[i], &matWorld);

		D3DXVec3TransformCoord(&m_vecSufaceDebug[i].p, &m_vecSufaceDebug[i].p, &matWorld);
	}
}
